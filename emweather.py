...
# [更新履歴]
#   2022/3/4 新規作成
#   2023/11/7 新バージョン作成・機能追加
#   2023/11/9 DBレイアウト変更に伴う修正
# ======================================
import datetime
import csv
...
def weather_list_get(prec,block,year,month): 
    ...
    data_list1 = []
    data_list2 = []
    for r in range(0,len(data_per_day)):
        #日付カラムを追加
        c_date = datetime.datetime(int(data_per_day[r][5]), int(data_per_day[r][6]), int(data_per_day[r][0]) , 0, 0, 0) 
        for c in range(0,len(data_per_day[r])): 
            data_list1.append(data_per_day[r][c])
        data_list1.append(c_date)
