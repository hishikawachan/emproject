...
class Mainmenu(object):
    def __init__(self, root_1):
        ...
        #self.radiobutton3 = tk.Radiobutton(root_1, text="期間別売上合計値照会(現金)", variable=self.var, value="1b3")
        self.radiobutton4 = tk.Radiobutton(root_1, text="売上情報取込み(期間設定)", variable=self.var, value="1b3")

        # ラジオボタンをウィンドウに配置
        self.radiobutton1.place(x=200, y=60)
        self.radiobutton2.place(x=200, y=80)
        #self.radiobutton3.place(x=200, y=100)
        self.radiobutton4.place(x=200, y=100)

        # 決定ボタン
        self.button_11 = tk.Button(root_1, text="決定", command=self.proc_selection) 
        self.button_11.place(x=250, y=140)

        # 終了ボタン
        self.button_12 = tk.Button(root_1, text="終了", command=lambda: on_exit(root_1))
        self.button_12.place(x=320, y=140)

        self.label_12 = tk.Label(root_1, text="実行する項目を選択し「決定」をクリックしてください")
        self.label_12.place(x=250, y=180)        
