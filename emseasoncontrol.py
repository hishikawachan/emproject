...
if __name__ == "__main__":    
    ...
    print('期間帳票処理開始：',datetime.datetime.now()) 
    ...
    sday = input('開始日を入力してください(その月の存在する日):') 
    eday = input('終了日を入力してください(その月の存在する日):')
    #debug  
    #print('期間指定帳票出力開始      :',datetime.datetime.now())
    ...
    # 指定された開始日、終了日をセット
    last_day = datetime.date(int(year), int(month), int(eday))
    first_day = datetime.date(int(year), int(month), int(sday))
